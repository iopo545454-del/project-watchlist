...
- 2026-07-21 — Dedicated verification closed the broad node/bridge-readiness check with a split result. The public `lite_node` remains an **internal-use pre-release**, now at unsigned commit `2a5803b`, with no release, tag, declared license, CI workflow or GitHub deployment; official validator docs say onboarding is paused during the mainnet-beta transition, and a repository search found no TAPE-named artifact. The June “next week” TAPE / two-week container schedule is therefore stale. Separately, the first-party Ethereum bridge is now primary-source verifiable: official docs publish canonical wOCT `0x4647...6E80`, `EthereumBridge` `0xE7eD...4caE`, `OctraLightClient` `0xC01c...F55d` and Octra `BridgeVault` `oct5Mr...SpCHq`; Ethereum Blockscout verifies the contracts and showed `197,789.585123` wOCT supply / `3,350` holders at the check. Public Octra RPC returned network `v3.0.0-irmin`, `1,358,347` active accounts and `172,099,908` transactions. No OCT/wOCT token exists in Hyperliquid's public spot metadata, and no bridge audit was found. The bridge is live/documented; a supported node, public TAPE artifact, audit and Hyperliquid spot remain absent. Sources: https://github.com/octra-labs/lite_node, https://docs.octra.org/validator-docs/running-a-node, https://docs.octra.org/oct-docs/contract-addresses, https://docs.octra.org/oct-docs/bridging, https://eth.blockscout.com/address/0x4647e1fE715c9e23959022C2416C71867F5a6E80, https://eth.blockscout.com/address/0xE7eD69b852fd2a1406080B26A37e8E04e7dA4caE, https://octra.network/rpc and https://api.hyperliquid.xyz/info
...
| Node release | Confirmed public preview / promised production schedule stale | Official X made the node a prerequisite: https://x.com/i/status/2066055111952990487. Current GitHub still describes `lite_node` as an internal-use pre-release, with no releases/tags/license/CI/deployments: https://github.com/octra-labs/lite_node. Official validator docs say onboarding is paused during the mainnet-beta transition: https://docs.octra.org/validator-docs/running-a-node | Public preview observed 2026-07-14; production/container timing unresolved as of 2026-07-21 | Inspectable code and public RPC exist, but there is still no supported, versioned operator release or reproducible release pipeline. | A documented/audited release could broaden validation; relying on an unsigned internal preview creates upgrade, reproducibility and support risk. | Reopen the release catalyst only for a supported tag/container, license, signed/reproducible build, configs, onboarding instructions and independent operator evidence. |
| First-party Ethereum bridge / wOCT and eventual Hyperliquid spot | Bridge confirmed live/documented; Hyperliquid spot absent | Official contract-address and bridging docs: https://docs.octra.org/oct-docs/contract-addresses and https://docs.octra.org/oct-docs/bridging. Ethereum explorer verifies wOCT and bridge contracts: https://eth.blockscout.com/address/0x4647e1fE715c9e23959022C2416C71867F5a6E80 and https://eth.blockscout.com/address/0xE7eD69b852fd2a1406080B26A37e8E04e7dA4caE. Hyperliquid public spot metadata had no OCT/wOCT match: https://api.hyperliquid.xyz/info | Bridge verified 2026-07-21; Hyperliquid spot date unknown | Provides a canonical 1:1 OCT↔wOCT route on Ethereum. At verification, wOCT supply was `197,789.585123` with `3,350` holders. | Canonical bridging improves Ethereum access, but bridge/admin/light-client security and thin wrapped supply matter; it does not establish Hyperliquid approval or liquidity. | Verify independent audit, role/admin custody, representative round trips, reserve/supply reconciliation and any official Hyperliquid market approval. |
...
| TAPE verifier / CT-format migration | Confirmed intent / June delivery schedule stale | Co-founder mini-roadmap said Octra was moving to a native TAPE verifier with client-side CT-format migration: https://x.com/lambda0xE/status/2066228923357139270. Verification found no TAPE-named artifact in the current public node tree or official docs: https://github.com/octra-labs/lite_node and https://docs.octra.org/ | “Next week” target from 2026-06-14 missed; no public release by 2026-07-21 | No public migration can yet be verified, so the security-gating dependency remains open rather than completed. | The missed schedule increases execution/security-process risk; a reviewed release could still restore confidence. | Reopen only for migration instructions, versioned code/release notes, wallet prompts, audit evidence and observable activation. |
...
|---|---|---|---|---|
| Supported node count, uptime and version mix | Tests decentralization/production readiness | Official peer/RPC/explorer data | partial / public RPC tested | `node_status` and `node_stats` are public, but they describe the canonical RPC/network rather than supported external-node count/version distribution. Internal mini-core execution is not supported mainnet operation. |
...
| OCT supply, bridge liquidity and emissions | Measures market structure | Canonical contracts/explorers | partial / canonical addresses tested | Official docs now map wOCT, EthereumBridge, OctraLightClient and BridgeVault; audit/admin controls, reserve reconciliation and Hyperliquid market access remain unresolved. |
...
| Octra public RPC | `https://octra.network/rpc` JSON-RPC (`node_status`, `node_stats`, `octra_supply`) | tested_ok | Version, epochs, accounts, transactions and supply fields | Add collector only after defining material network-activity thresholds and validating historical semantics. |
| Canonical bridge addresses | `https://docs.octra.org/oct-docs/contract-addresses` plus Ethereum explorer reads | tested_ok | wOCT, bridge, light-client, auction and Octra vault addresses; wOCT supply/holders | Audit roles, reconcile wOCT supply to locked OCT and test representative round trips. |
...
| https://docs.octra.org/oct-docs/contract-addresses | Official canonical wOCT, EthereumBridge, OctraLightClient, auction and BridgeVault addresses. |
| https://docs.octra.org/oct-docs/bridging | Official 1:1 OCT↔wOCT bridge flow, finalization, fee and trust-model documentation. |
| https://eth.blockscout.com/address/0x4647e1fE715c9e23959022C2416C71867F5a6E80 | Verified Ethereum wOCT contract and live supply/holder surface. |
| https://octra.network/rpc | Public Octra JSON-RPC; tested for node/network and supply statistics. |

## Open questions
- The June TAPE/node schedule is stale. Reopen only when versioned TAPE migration artifacts and a supported node/container ship beyond the internal-use preview, with license, signed/reproducible builds, configs, onboarding and independent review.
- Canonical first-party OCT/wOCT bridge contracts are now documented. What audits, admin/pauser/bridge-role custody, reserve reconciliation and representative round trips support them, and what exact prerequisites remain for Hyperliquid spot?
