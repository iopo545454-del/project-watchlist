...
- 2026-07-25 — Dedicated verification closed the post-Harvard / dstack bundle as **stale rather than carrying an unmeasurable attribution task**. The three complete pre-close API days (July 21–23) totaled `$45,629.24` revenue, `21,998,468` requests and `150.919B` tokens, or `$302.34K` revenue per trillion tokens. July 24, whose exact shutdown hour was not disclosed, was `7.64%` below the pre-window daily revenue average and `7.93%` lower on tokens while revenue/T was effectively flat (`+0.31%`); July 25 remained an incomplete/mutable post-close day at verification. Both public APIs aggregate all routes and expose no hostname, opt-in, customer or discount dimension, so they cannot isolate Harvard traffic. The old hostname still answered auth-gated requests after the deadline rather than disappearing at DNS/edge level. dstack's developer-preview gateway can fetch public Chutes TDX/GPU evidence, but its README says receipts are in-memory and a public transparency log is not implemented; no official public gateway deployment, Chutes-routed receipt or attributed traffic ledger exists. Reopen only on endpoint-attributed history or a public gateway report/receipt plus Chutes traffic. Sources: https://api.chutes.ai/daily_revenue_summary, https://api.chutes.ai/invocations/stats/llm?start_date=2026-07-21&end_date=2026-07-25, https://research-data-opt-in.chutes.ai/v1/models and https://github.com/Dstack-TEE/private-ai-gateway
- 2026-07-23 — dstack merged **private-ai-gateway PR #101**, which now publishes the exact measured `app_compose` preimage in ACI attestation evidence and verifies its SHA-256 hash against the RTMR3-bound `compose-hash`. Because Chutes is a supported verified upstream in this gateway, the change makes the workload manifest behind a Chutes route independently checkable instead of relying only on a high-level confidential-compute flag. The repository explicitly leaves reviewed source/image acceptance as verifier-policy TODOs, and this does not prove a Chutes production deployment, customer adoption or attributed inference volume. Source: https://github.com/Dstack-TEE/private-ai-gateway/commit/adfda25f436f9b0172b68e69b62069eef8617b83
- 2026-07-22 — Chutes said the **Harvard research data-collection phase ends July 24**: the opt-in `research-data-opt-in.chutes.ai` endpoint will be removed, its 25% inference discount will end and prompt/response recording will stop; users must return to `llm.chutes.ai/v1` with existing API keys. This closes a known discount/data-recording program and creates a hard migration deadline, but Chutes does not disclose endpoint-attributed traffic or revenue, so the effect on paid demand, revenue per token and margins is unknown. Current same-window DefiLlama moves remained below configured materiality thresholds. Sources: https://x.com/chutes_ai/status/2080012673689801098, https://x.com/chutes_ai/status/2031807407437889968, https://research-data-opt-in.chutes.ai/v1/models and https://llm.chutes.ai/v1/models
...
- Harvard opt-in program closure and migration notice: https://x.com/chutes_ai/status/2080012673689801098
...
| Confidential-inference verification and adoption | Tests whether Chutes' TEE route has independently checkable workload evidence and real demand | Chutes evidence API, dstack ACI attestation reports/source and partner-attributed traffic | partial / recurring check closed stale | Public Chutes TDX/GPU evidence is retrievable, but dstack `0.1.0` is a developer preview with in-memory receipts and no public transparency log or official public gateway deployment. Reopen only on a public base URL/report/receipt and attributed Chutes traffic. |
...
| dstack private AI gateway | `https://github.com/Dstack-TEE/private-ai-gateway/commit/adfda25f436f9b0172b68e69b62069eef8617b83` | tested_ok / deployment unverified | Chutes-capable upstream verification plus measured `app_compose` evidence bound to the attested Compose hash | Verify a live deployed report against reviewed source/image policy, then measure Chutes-routed requests, tokens, fees and failures. |
...
| TEE plus GraVal GPU verification and confidential API positioning | Confirmed / technical moat; partner attestation path advancing | Official Chutes posts: https://x.com/chutes_ai/status/2047314719707422754 and https://x.com/chutes_ai/status/2070567809713606938. dstack gateway evidence update: https://github.com/Dstack-TEE/private-ai-gateway/commit/adfda25f436f9b0172b68e69b62069eef8617b83 | Live / rolling infrastructure upgrade | Improves trust that miners provide claimed hardware and says TEE-backed serving keeps prompts/outputs private from GPU operators; dstack's gateway now exposes the measured Compose preimage and verifies it against the RTMR3-bound hash for supported routes including Chutes. | Stronger independently checkable workload evidence can support enterprise/privacy demand, but reviewed source/image acceptance remains policy work and no Chutes-attributed traffic is public. | Verify a live Chutes-routed attestation against reviewed source/image policy; watch audits, deployment/adoption, attributed traffic, Blackwell coverage, slashing stats and any TEE or miner-cheating incidents. |
...
## Open questions
- The Harvard endpoint-effect check is closed stale: public revenue/throughput APIs aggregate every route and no complete endpoint-attributed pre/post history exists. Reopen only if Chutes publishes a hostname/discount-attributed series; continue the broader question of discounts, free quotas and incentive costs outside the revenue table.
...
- Which integrations route meaningful recurring volume to Chutes? dstack routing, measured-Compose code and public Chutes TDX/GPU evidence are source-confirmed, but no official public gateway deployment, durable receipt/transparency ledger or dstack/ChronoSeek-attributed volume exists. Reopen the closed gateway task only on those artifacts.
