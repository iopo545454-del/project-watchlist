...
- 2026-07-22 — Chutes said the **Harvard research data-collection phase ends July 24**: the opt-in `research-data-opt-in.chutes.ai` endpoint will be removed, its 25% inference discount will end and prompt/response recording will stop; users must return to `llm.chutes.ai/v1` with existing API keys. The opt-in endpoint still returned an authenticated-service response at the scan while the primary model endpoint remained live. This closes a known discount/data-recording program and creates a hard migration deadline, but Chutes does not disclose endpoint-attributed traffic or revenue, so the effect on paid demand, revenue per token and margins is unknown. Current same-window DefiLlama moves remained below configured materiality thresholds. Sources: https://x.com/chutes_ai/status/2080012673689801098, https://x.com/chutes_ai/status/2031807407437889968, https://research-data-opt-in.chutes.ai/v1/models and https://llm.chutes.ai/v1/models
