...
|---|---|---|---|---|
| EigenLayer/EigenCloud fees and protocol revenue | Tests whether services create economic output and whether any value is retained by the protocol | DefiLlama fees/revenue adapters plus official fee-recipient contracts | tested_ok / attribution partial | As of 2026-07-22, supplier-side rewards are `$674` 24h / `$1,765` 7d / `$629,785` 30d while protocol revenue is `$0`; the seven-day series recovered `60.75%` from an in-window `$1,098` low but remained `54.60%` below the pre-window `$3,888` snapshot. Restaking rewards are not automatically EigenCloud revenue, Darkbloom revenue or EIGEN value capture. |
...
|---|---|---|---|---|---|---|---|
| 2026-07-16 | Darkbloom's PR #548 reported 23,134 Gemma tool-template 500s—42.4% of Gemma tool terminal attempts in a 24-hour production window—with retries reaching 17 attempts and zero eventual successes for that class; signed v0.7.11 shipped the first fix, and open PR #561 now adds inference-time grammar enforcement plus capability-gated/fail-closed routing. | https://github.com/Layr-Labs/d-inference/pull/548; signed release: https://github.com/Layr-Labs/d-inference/releases/tag/v0.7.11; follow-up enforcement PR: https://github.com/Layr-Labs/d-inference/pull/561 | Official GitHub PR / production telemetry | High | Partially supported — v0.7.11 is signed/notarized and PR #561 materially expands enforcement, but #561 is unmerged and canary/production rollout plus post-fix error rates remain unverified | This is a quantified live-product reliability failure that can waste fleet capacity, break tool-using agents and unfairly penalize healthy providers. | Track PR #561 merge/release and protocol-v1 provider adoption; verify coordinator/provider canary and broad rollout, then measure post-fix error rate, failed-user impact and provider-payout effects. |
...
- When do signed v0.7.11 and PR #561's inference-time tool constraints reach coordinator/provider canary and broad production, and what are the resulting error rate, paid share of the reported `~3B` daily tokens, customer concentration, provider quality/revenue, model mix and EIGEN economics? When do MTP and exact-cache routing activate separately?
