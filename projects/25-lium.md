...
- 2026-07-26 — Dedicated verification closed PR #1159's production-impact check as **stale/unattributable because the fix is not on Lium's production validator branch**. The merged commit `7d083ad` is on `main`, while `prod_validator` remains at `eabd545` from February 9 and does not contain the fix; the latest public production-validator workflow run is April 12. Current API movement therefore cannot be credited to #1159. GPU inventory moved from `659 / 974` rented (`67.66%`) to `675 / 1,140` (`59.21%`): only `+16` rented GPUs against `+166` total supply, a `-8.45pp` utilization change. July validator-attributed revenue reached `474,711.54`, up `90,553.26` / `23.57%` from the July 21 snapshot, but the endpoint exposes no release/placement attribution. Reopen #1159 only after a production deployment SHA plus before/after affected-executor, placement/failure and paid-hours evidence. Sources: https://github.com/Datura-ai/lium-io/pull/1159, https://github.com/Datura-ai/lium-io/tree/prod_validator, https://github.com/Datura-ai/lium-io/actions/runs/24317016590, https://lium.io/api/executors/stats and https://lium.io/api/billing/revenue-for-validators
...
| Official validator-attributed rental revenue | `$367,464.20` May; `$284,191.33` June; `$474,711.54` July-to-date | Lium public API as of 2026-07-26; docs define renter USD payments and a `95%` provider / `5%` Lium split, but the endpoint has no explicit response schema or release attribution |
| Live GPU utilization | `675 / 1,140` rented (`59.21%`) | Lium public `/api/executors/stats` as of 2026-07-26; versus `659 / 974` (`67.66%`) on July 23, supply rose by 166 while rentals rose by 16, reducing utilization by 8.45 percentage points |
...
|---|---|---|---|---|
| GPU inventory and utilization by class | Distinguishes real rental demand from idle supply | Lium `/api/executors/stats` | tested_ok | Public endpoint returned `675 / 1,140` rented as of 2026-07-26; it is a point-in-time snapshot and does not identify customer concentration. PR #1159 is absent from the production validator branch, so movement cannot be attributed to the fix. |
...
| High-end GPU inventory and Blackwell availability | Confirmed live API; PR #1159 merged to `main` but production impact stale/unproven | Official API: https://lium.io/api/executors/stats; code: https://github.com/Datura-ai/lium-io/pull/1159; production branch: https://github.com/Datura-ai/lium-io/tree/prod_validator | Utilization verified 2026-07-26; fix absent from production branch | Public data showed `675 / 1,140` GPUs rented (`59.21%`). PR #1159 can remove a UFW port-verification cap, but the merged commit is not on `prod_validator`. | Supply increased much faster than rentals after the prior snapshot; no public evidence ties placements, paid hours, failures or revenue to the undeployed fix. | Reopen the fix only on a production deployment SHA plus before/after affected-executor, placement/failure and paid-hours evidence; continue ordinary utilization/revenue history separately. |
...
- How concentrated and durable are the `675 / 1,140` rented GPUs by customer, paid hours and cohort? PR #1159's production-impact check closed stale on 2026-07-26 because the merged commit is absent from `prod_validator`; reopen only after a production deployment SHA and before/after affected-executor, placement/failure and paid-hours evidence.
