...
| Live GPU utilization | `603 / 957` rented (`63.01%`) | Lium public `/api/executors/stats` as of 2026-07-21 |
...
|---|---|---|---|---|
| GPU inventory and utilization by class | Distinguishes real rental demand from idle supply | Lium `/api/executors/stats` | tested_ok | Public endpoint returned 21 GPU classes and `603 / 957` rented as of 2026-07-21; it is a point-in-time snapshot and does not identify customer concentration or paid hours |
...
| High-end GPU inventory and Blackwell availability | Confirmed / live API-backed utilization | Official API: https://lium.io/api/executors/stats; official July operating update: https://x.com/lium_io/status/2077877053496467804 | Verified 2026-07-21 | Public data showed `603 / 957` GPUs rented, including A100 `97.96%`, B200 `51.02%`, B300 `25.0%` and RTX PRO 6000 Blackwell Server `74.75%`. | API-backed utilization materially strengthens demand evidence, though concentration, retention and net margin remain unknown. | Add history, customer concentration/retention, failure-rate and paid-hours context; reconcile revenue with provider payout and burns. |
...
- How concentrated and durable are the `603 / 957` rented GPUs by customer, paid hours and cohort, and do reported failure-rate improvements persist?
