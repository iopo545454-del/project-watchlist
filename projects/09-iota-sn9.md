...
- **2026-07-23 — Dedicated verification closed the broad Orion/dashboard/economics check as stale.** The official repository added tag **`v4.8.0`**, but its entire `v4.7.0...v4.8.0` diff is a nine-line `src/miner/main_pool.spec` packaging change; GitHub still exposes no post-`v3.0.0` release, model checkpoint, Orion benchmark or cost ledger. Searches of the official GitHub organization and Hugging Face model/dataset/space APIs found no Macrocosmos Orion/IOTA checkpoint. The public dashboard remains a reachable app shell without a documented stable run/reward API, and the official Train at Home FAQ describes eligibility/payout mechanics rather than retention, rewards or paid-run history. The standing omnibus check is removed from the autonomous queue; reopen only when a checkpoint/report, documented dashboard endpoint, payment ledger or independent reproduction creates a bounded task. Sources: https://github.com/macrocosm-os/iota/compare/v4.7.0...v4.8.0, https://github.com/macrocosm-os/iota/releases and https://docs.macrocosmos.ai/product-and-services/tah/faqs
...
|---|---|---|---|---|---|---|
| Project Orion next-stage model release | Tentative / stale until an artifact ships | Macrocosmos co-founder/CTO said the next model release was coming “very soon”: https://x.com/macrocrux/status/2077493082388324483. Dedicated verification found no matching checkpoint/release in official GitHub or Hugging Face as of 2026-07-23: https://github.com/macrocosm-os/iota/releases | No firm date; first surfaced 2026-07-15 | Could provide a new checkpoint/report for testing distributed quality, speed and cost claims. | A reproducible result could attract buyers, miners and validator capital; another issuer-only benchmark would add narrative without resolving economics. | Reopen only for an official checkpoint/report/code artifact with evals, cost, wall-clock speed, hardware mix or third-party reproduction. |
...
| IOTA dashboard and open-source training stack | Confirmed / ongoing | Dashboard, repository, current tag and primer: https://iota.macrocosmos.ai/dashboard/mainnet, https://github.com/macrocosm-os/iota, https://github.com/macrocosm-os/iota/tree/v4.8.0 and https://arxiv.org/abs/2507.17766 | Live; latest code tag `v4.8.0` dated 2026-07-23 as of this review | Makes miner/validator mechanics and some training progress inspectable; v4.8.0 changes only the miner packaging spec after v4.7.0's Bittensor compatibility work. | Stable machine-readable history and reproducible releases could improve underwriting; maintenance tags without model/economic artifacts do not prove demand. | Reopen API/run-history work only for a documented endpoint; monitor versioned model/checkpoint/eval artifacts. |
...
- The broad Orion reproduction/dashboard/economics task was closed stale on 2026-07-23 because no bounded checkpoint, documented API or payment ledger exists. Reopen model-quality, 65%-of-centralized-speed and cost work only when Macrocosmos publishes a checkpoint/report or a third party publishes reproducible logs.
- Reopen paid-run/revenue work only for a documented run/payment ledger; one 5-TAO demo does not establish repeat demand or gross margin after contributor rewards and infrastructure costs.
- Reopen Train at Home cohort/retention/reward concentration work only for machine-readable history or a primary payout ledger.
...
- The “very soon” Orion model/checkpoint claim has no firm date and is stale for autonomous verification; monitor for an actual code/checkpoint/eval artifact rather than carrying the same check.
