...
last_updated: 2026-07-22T23:38:43Z
---
...
- 2026-07-22 — **NEAR completed its first automatic mainnet shard split**, closing the key production-proof check for Protocol v86 dynamic resharding. Official NEAR said a state-size threshold triggered the transition from 9 to 10 shards without manual coordination. Archival mainnet RPC independently pins the epoch-boundary transition between blocks `207,849,133` and `207,849,134` at `2026-07-21T01:15:14Z`: shard `5` was replaced by child shards `12` and `13`, protocol version stayed `86`, and the current finalized layout still had 10 shards at the scan. The chain evidence proves the layout transition and continued block production; the official no-fork/no-pause claim is not a substitute for a full validator/client incident audit. Sources: https://x.com/NEARProtocol/status/2080018646630355150, https://x.com/ilblackdragon/status/2080025985869881562 and https://archival-rpc.mainnet.near.org
...
| Post-quantum signing, dynamic resharding, and SPICE upgrades | Confirmed / v86 live; first automatic split completed; SPICE pending | Official mainnet announcement: https://x.com/NEARProtocol/status/2079189515843821764. First-split confirmation: https://x.com/NEARProtocol/status/2080018646630355150. Canonical releases: https://github.com/near/nearcore/releases/tag/2.13.1 and https://github.com/near/nearcore/releases/tag/2.13.0. Archival RPC maps the transition at block `207,849,134`; merged epoch config: https://github.com/near/nearcore/pull/15823 | Protocol v86 live 2026-07-20; first automatic split 2026-07-21; SPICE timing still “coming months” | Makes FIPS-204 ML-DSA-65 a production transaction/access-key scheme and has now executed the first threshold-driven epoch-boundary split, replacing shard `5` with child shards `12`/`13` and increasing mainnet from 9 to 10 shards. | The clean observable transition strengthens NEAR's production-scaling case and reduces manual shard-layout coordination; future splits/client incidents and real ML-DSA adoption remain the durability tests. | Monitor 10-shard health, validator/client incidents and later split attempts; measure ML-DSA key/wallet adoption, then SPICE releases/benchmarks. |
...
| Protocol/client activation | Tracks v86, post-quantum keys, dynamic resharding and SPICE | nearcore releases, governance and mainnet RPC | tested_ok for live protocol/client version and first split; broader operations manual | RPC directly confirms protocol `86`, node `2.13.1` and the first 9→10-shard transition at block `207,849,134`; ML-DSA adoption and broad validator/client health still require history and event-level checks. |
...
| NEAR mainnet RPC | `block` + `status` + `EXPERIMENTAL_protocol_config` at current and archival mainnet RPC | tested_ok | Live protocol/client version, historical/current shard layout and runtime costs including ML-DSA verification | Persist lightweight protocol/layout history; monitor later splits because the legacy genesis `dynamic_resharding` boolean is not the epoch feature gate. |
