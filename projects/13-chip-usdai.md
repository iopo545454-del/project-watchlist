...
- Reconcile the issuer's renewed `$400M+` TVL claim with DefiLlama's approximately `$203.3M` TVL plus `$157.9M` borrowed categories; verify the above-8% sUSDai APY, Lighthouse ARR, deployed-vs-idle liquidity, loan concentration, escrow attestations, NIM durability and defaults/delinquencies.
- GPU-backed private credit has real-world collateral, duration, and liquidation risk; headline APY is not risk-free.
- Team is partly pseudonymous; prioritize official reports, attestations, and verifiable loan performance.
