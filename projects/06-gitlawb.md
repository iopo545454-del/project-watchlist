...
last_updated: 2026-07-25T21:17:21Z
---
...
## Overview
**Identity**
- Likely correct spelling: **GitLawb**.
- Project describes itself as “the git layer for the AI-native internet,” with DIDs over accounts and signed commits by agents/humans.
- Token commonly referenced as `$GITLAWB`, reportedly on Base/community contexts.

**Official links**
- X: https://x.com/gitlawb
- Website: https://gitlawb.com
- Token page: https://gitlawb.com/token
- Official Telegram channel reportedly: https://t.me/gitlawbofficial
- Related/intern account: https://x.com/gitlawb_intern

**Team/key people**
- Kevin — founder, surfaced as https://x.com/kevincodex.
- Small/founder-led team; public identities beyond Kevin are less clear.
- Intern/mirroring account: https://x.com/gitlawb_intern.

**Investors/backers**
- No clear institutional investors confirmed from searched sources.
- Community/builder-led project; monitor treasury/grants announcements if relevant.

**KOL/community accounts**
- https://x.com/gitlawb
- https://x.com/kevincodex
- https://x.com/gitlawb_intern
- Community explainers/KOLs surfaced: https://x.com/Outpost_HGTP, https://x.com/GemsHunting_8 — secondary/non-official sources.
...
**Alert triggers**
- Official contract/tokenomics/staking updates.
- Node incentives, grants, or treasury programs.
- Major product releases: OpenClaude, OpenGateway, Playground.
- Security incidents involving GitHub PATs, DIDs, signed commits, or mirrors.
- New official social/channel announcements.

**Notes/ambiguities**
- User spelling “GitlWab/GitLwab/Gitwab” is ambiguous; current best match is **GitLawb** at @gitlawb.
- Project itself has warned about fake Telegram groups; only trust channels linked by official X/site.

---

## Latest scan notes

- 2026-07-25 — Dedicated verification independently exercised **Node/CLI v0.7.0 end to end**: a fresh `gl init` registered an external DID, created a public repository, accepted two pushes, issued a node-signed ref-update certificate and passed `gl cert show --verify`; the artifact remains public at the node API. The four explorer-configured nodes expose `3,973` normalized agent DIDs and `12,087` normalized owner/repository pairs, but only `4` agents and `8` repositories appeared after the v0.7.0 release—and one of each is this verification artifact—so totals include old/spam state and are not a retained-user cohort. Of `13` peer URLs marked reachable across those registries, `12` returned `/health` 200 and one was blocked by a Cloudflare 403. npm recorded `173` `@gitlawb/gl` package requests on July 23–24; GitHub showed `3,226` binary asset requests, `3,200` concentrated in Linux x64 with nearly matching checksum requests, so neither is a unique-install count. The buyback vault advanced from `26` to `49` flushes and from `90.144M` to `180.964M GITLAWB` processed/burned at `10,000` burn bps, still with zero forwarded; upstream USDC/product attribution remains unavailable. This confirms the release workflow and bounded network use, closes the omnibus adoption check and leaves only event-driven cohort/revenue attribution questions. Sources: https://node.gitlawb.com/api/v1/repos/z6MkrW9fxasezg8vinBVDABsvqxhNEMHxvp9kAf6u86RRJng/external-v070-verification-20260725, https://node.gitlawb.com/api/v1/stats, https://api.github.com/repos/Gitlawb/node/releases/tags/v0.7.0, https://api.npmjs.org/downloads/range/2026-07-23:2026-07-25/%40gitlawb%2Fgl and https://base.blockscout.com/address/0xA05174583791bA7fecb37278A4d5c97C1E24Fc5f
- 2026-07-23 — GitLawb shipped **Node/CLI v0.7.0** and published a raw agent-first operating guide tested against a live node. The release adds real signature and trusted-issuer verification for ref-update certificates through `gl cert show --verify`, Windows support, improved `gl init`/status/doctor flows and automated npm/Homebrew/Docker release provenance. Official X says a fresh AI agent used only the guide to publish a repository and verify its receipt in about four minutes on a second run after the first run exposed onboarding gaps. The tag and guide are reproducible; the four-minute test remains issuer-run and the release discloses no node count, external retained users, paid flow or GITLAWB value delta. Sources: https://x.com/gitlawb/status/2080115905728790992, https://github.com/Gitlawb/node/releases/tag/v0.7.0 and https://explorer.gitlawb.com/docs/agents.md
- 2026-07-22 — Oldest-first verification confirmed that GitLawb's previously unverified product-to-token route is now executing on Base. The verified `GitlawbBuybackVault` at `0xA05174583791bA7fecb37278A4d5c97C1E24Fc5f` is configured for a **100% burn**, and direct contract reads showed **26 flushes / 90,144,399.820766 GITLAWB processed and burned**, zero forwarded and zero pending. The source says an offchain bot buys GITLAWB with protocol-revenue USDC, with x402 OpenGateway top-ups first, but the chain only proves bought tokens entering the vault and going to `0x…dEaD`; it does not prove the originating USDC amount, Ads/OpenGateway attribution, gross revenue, provider costs or retained margin. Adoption is measurable but early: the public npm API recorded **972 `@gitlawb/zero` downloads** and **22 `@gitlawb/gl` downloads** from July 15–22, while release assets had only 26 Zero and 5 Node binary downloads at verification. Kiln remains a live but closed implementation, and the Ads surface still exposes rounded clicks rather than exact CTR, itemized advertiser budgets or repeat cohorts. This closes the broad verification item; reopen revenue attribution only for a stable ledger/API or wallet-mapped USDC inflows. Sources: https://github.com/Gitlawb/contracts/blob/main/src/GitlawbBuybackVault.sol, https://base.blockscout.com/address/0xA05174583791bA7fecb37278A4d5c97C1E24Fc5f, https://api.npmjs.org/downloads/point/2026-07-15:2026-07-22/@gitlawb%2Fzero and https://api.npmjs.org/downloads/point/2026-07-15:2026-07-22/@gitlawb%2Fgl
- 2026-07-22 — Founder Kevin's GitLawb Ads snapshot and the live `ads.gitlawb.com` page showed about **548,000 impressions served, 12,000 clicks, $474.93 paid to viewers and 11 live campaigns**. Versus the July 14 snapshot, impressions rose about **62.6%**, viewer payouts **40.8%** and campaigns **22.2%**, while rounded clicks were unchanged; implied click-through fell from roughly **3.56% to 2.19%**. This advances ad-distribution telemetry but still does not expose advertiser spend, revenue, repeat advertisers, paid-versus-promotional mix or `$GITLAWB`/USDC routing. Sources: https://x.com/kevincodex/status/2079857761769439669 and https://ads.gitlawb.com/
- 2026-07-22 — GitLawb shipped two versioned open-source releases in the same product cycle. **Zero v0.5.0** is live with concurrent read-only tool batches, prompt caching/prewarming, a unified sandbox and stronger security checks, `/undo`, isolated `/btw` conversations, transactional extension installs and runtime/provider reliability work. Separately, **Node v0.6.0** added iCaptcha proof-of-work solving and fixed wildcard-capability handling, private-repository metadata exposure on `/hooks` and sibling read surfaces, terminal bidi/control sanitization and ref-update identity propagation. These are concrete developer/network hardening releases, but neither discloses external active users, node adoption, paid usage, `$GITLAWB` routing or a token-value change. Sources: https://x.com/gitlawb/status/2079748230771048796, https://github.com/Gitlawb/zero/releases/tag/v0.5.0 and https://github.com/Gitlawb/node/releases/tag/v0.6.0
- 2026-07-20 — Founder Kevin introduced **Kiln**, a live `playground.gitlawb.com/kiln` surface described only as the lightweight harness powering GitLawb Playground. Direct HTTPS returned the live page, but the announcement provides no repository, license, architecture, benchmarks, usage, pricing or `$GITLAWB` economics. This is a concrete developer-product release worth tracking in the raw feed, but its scope and adoption are still unproven. Source: https://x.com/kevincodex/status/2079319114376712657 and https://playground.gitlawb.com/kiln
- 2026-07-20 — Official GitLawb made **Kimi K3 live on OpenGateway and for Playground Premium users**, then disclosed an additional **$9,487.30 XiaomiMiMo bonus-balance / $9,000+ inference-credit grant** to support MiMo traffic, access and builders on OpenGateway. This broadens the live model/provider surface and adds a named external infrastructure subsidy, but it does not establish paid demand, cash revenue, margin, user retention or `$GITLAWB` value capture; sponsored inference must be separated from organic traffic in the cumulative dashboard. Sources: https://x.com/gitlawb/status/2079057764945727783 and https://x.com/gitlawb/status/2079071618421404149
...
- 2026-07-01 — Founder Kevin said the team is preparing a Zero release alongside Fable 5 access after Anthropic model access constraints changed. This is a product-release teaser, not yet a live launch; next checks are official Zero release notes, model availability, cost/latency, and whether it drives OpenClaude/OpenGateway usage. Source: https://x.com/kevincodex/status/2072125600496775557

- 2026-06-30 — Official GitLawb said LongCat is coming to OpenClaude and Zero while quoting a CoinBureau post about Meituan's LongCat 2.0 model. This is a product/model-support signal for OpenClaude/OpenGateway rather than tokenomics; next checks are whether the model actually appears in the app, provider/latency/cost details, usage metrics, and whether it pulls demand into paid credits or ads. Source: https://x.com/gitlawb/status/2071876931117785389
...
|---|---|---|---|---|---|---|
| Agent-native decentralized Git / Node v0.7.0 | Confirmed / completed and independently exercised | Official release/guide: https://github.com/Gitlawb/node/releases/tag/v0.7.0 and https://explorer.gitlawb.com/docs/agents.md. Independent public artifact: https://node.gitlawb.com/api/v1/repos/z6MkrW9fxasezg8vinBVDABsvqxhNEMHxvp9kAf6u86RRJng/external-v070-verification-20260725 | v0.7.0 live 2026-07-23; verified 2026-07-25 | A fresh install registered a DID, created/pushed a repository, issued a signed certificate and verified it. Registries exposed `3,973` normalized DIDs / `12,087` repos, but only `4` agents and `8` repos appeared post-release and one of each was the verifier. | The workflow works outside the issuer's demonstration, but retention, unique installs and paid use remain unproven; download counts are request counts and are highly concentrated. | Closed as a recurring omnibus check. Reopen only on a stable active/retained-user cohort, version-share metric, material external adoption or a fee-linked production artifact. |
| Protocol-revenue GITLAWB buyback-and-burn vault | Confirmed / live onchain; revenue-source attribution partial | Verified source and contract state: https://github.com/Gitlawb/contracts/blob/main/src/GitlawbBuybackVault.sol and https://base.blockscout.com/address/0xA05174583791bA7fecb37278A4d5c97C1E24Fc5f | Deployed 2026-07-17; latest verification 2026-07-25 | The owner-controlled vault remains at `10,000` burn bps and has processed/burned `180,964,104.671626 GITLAWB` over `49` flushes, with zero forwarded—up from `90,144,399.820766` / `26` flushes on July 22. | Executed token removal is reproducible, but the offchain buys still do not prove Ads/OpenGateway/x402 gross USDC revenue, net revenue or margin. | Track wallet-mapped USDC inflows/swaps, burn cadence/notional versus supply, owner/burn-bps changes, source composition, provider costs and whether the FeeDistributor/staking stack passes external audit and deploys. |
| Kiln lightweight Playground harness | Confirmed / live product surface; implementation opaque | Founder announcement and live page: https://x.com/kevincodex/status/2079319114376712657 and https://playground.gitlawb.com/kiln | Live 2026-07-20 | Exposes the lightweight harness said to power Playground as a named product surface. | A reusable harness could make GitLawb's agent/developer stack easier to adopt outside the full Playground; without code, docs, users or economics it may remain an internal wrapper with a public page. | Verify repository/license, architecture, supported workflows/models, external usage, pricing/credits, security and any OpenGateway or GITLAWB route. |
| OpenGateway inference-routing usage and economics | Confirmed / ongoing issuer dashboard and live model surface | Official usage post/dashboard plus Kimi K3 availability and XiaomiMiMo credit support: https://x.com/gitlawb/status/2079017128502722711, https://gitlawb.com/opengateway/usage/global, https://x.com/gitlawb/status/2079057764945727783 and https://x.com/gitlawb/status/2079071618421404149 | Ongoing; latest snapshot 2026-07-20 | Reports `2.6206T` all-time tokens across `28.2M` requests since May 14, `85.3%` cache hits / `2.2T` cached tokens and `$219.6K` compute routed at list price; Kimi K3 is now live and XiaomiMiMo supplied a `$9,487.30` bonus credit balance. | A broader model surface and provider subsidy can improve access and builder distribution, but subsidized/cache-served traffic with no margin or token route can inflate usage without proving demand. | Identify a stable API/export; separate organic paid traffic from free/sponsored credits, then reconcile users, retention, models/providers, gross spend, cache accounting, provider cost, retained revenue and any `$GITLAWB` sink or fee route. |
...
| Zero agentic-coding release | Confirmed / completed live release | Official v0.5.0 announcement and canonical release: https://x.com/gitlawb/status/2079748230771048796 and https://github.com/Gitlawb/zero/releases/tag/v0.5.0 | v0.5.0 live 2026-07-22 | Ships a versioned Go terminal agent with concurrent read-only tool batches, caching/prewarming, unified sandbox enforcement, `/undo`, isolated `/btw` conversations and extension/runtime hardening. | A credible open-source coding agent can pull developers into GitLawb's broader node, OpenGateway and ads/credits stack; without usage or economics it can also remain a standalone GitHub project. | Measure installs, active users, retention, external contributions, model/provider usage and whether Zero routes demand into OpenGateway, credits, ads or `$GITLAWB`; verify Fable 5 separately because v0.5.0 release notes do not establish it. |
...
| `$GITLAWB` / USDC ad spend across OpenClaude, Playground, Codex, and agent-tool funding paths | Confirmed / live | Official GitLawb says advertisers can use `$GITLAWB` or USDC to run ads across OpenClaude and Playground; the live dashboard and founder snapshot showed about `548,000` impressions, `12,000` clicks, `$474.93` paid to viewers and `11` live campaigns: https://x.com/kevincodex/status/2079857761769439669, https://ads.gitlawb.com/, https://x.com/gitlawb/status/2070174891073261795 and https://x.com/gitlawb/status/2077165940815622455 | Live; latest telemetry 2026-07-22 | Versus July 14, impressions rose `62.6%`, payouts `40.8%` and campaigns `22.2%`, but rounded clicks were unchanged and implied CTR fell from about `3.56%` to `2.19%`. | Distribution is growing faster than clicks or viewer payouts; without advertiser spend, repeat-budget and token-route data, the loop can scale impressions without proving revenue quality or `$GITLAWB` demand. | Verify exact clicks/CTR, advertiser spend, CPM/CPC, paid-versus-promotional mix, repeat campaigns, `$GITLAWB` versus USDC funding, earned-credit balances, and whether token spend is burned, retained, or recycled. |
...
| `$GITLAWB` community proposals, development funding, and proposal fee share | Tentative / founder-described mechanics; not live | Founder says holders with at least $1,000 of `$GITLAWB` will be able to propose features, supported tokens, memes, or products; the community can vote to fund development and supporters can earn a share of fees from successful proposals: https://x.com/kevincodex/status/2076212086540238911 and https://x.com/kevincodex/status/2076272990480986340 | Upcoming / no launch date | Could add a token-gated proposal surface, community-directed development funding, and a fee-linked reward path. | If implemented with credible treasury and fee routing, governance could become a real `$GITLAWB` utility/value-capture loop; opaque weighting or discretionary payouts would make it engagement tooling instead. | Verify launch, proposal UI, voting eligibility/weighting, onchain vs offchain execution, treasury source/controls, supporter eligibility, fee-share formula, contracts, and first funded proposal. |
...
| https://github.com/Gitlawb/node/releases/tag/v0.6.0 | Canonical 2026-07-22 node release; iCaptcha support plus authorization, private-metadata, terminal-sanitization and reliability fixes. |
| https://github.com/Gitlawb/node/releases/tag/v0.7.0 | Canonical Node/CLI v0.7.0 release; signed-certificate verification, agent onboarding, Windows support and release-pipeline changes. |
| https://explorer.gitlawb.com/docs/agents.md | Raw agent operating guide tested against `gl` 0.7.0 and a live node; useful for repeatable identity, push, clone and receipt-verification checks. |
| https://github.com/Gitlawb/zero | Canonical open-source Zero agentic-coding repository; use versioned releases rather than social summaries for shipped functionality. |
| https://github.com/Gitlawb/zero/releases/tag/v0.5.0 | Canonical Zero v0.5.0 release notes; source for shipped sandbox, TUI, plugin, caching and reliability changes. |
| https://github.com/Gitlawb/contracts/blob/main/src/GitlawbBuybackVault.sol | Canonical buyback-vault source; defines the offchain USDC-buy description, owner controls, burn-bps setting and permissionless flush accounting. |
| https://base.blockscout.com/address/0xA05174583791bA7fecb37278A4d5c97C1E24Fc5f | Verified Base buyback vault; use contract reads and token transfers to reproduce processed/burned GITLAWB and configuration. |
...
| Credits, ads and `$GITLAWB` routing | Tests whether product usage reaches the token or treasury | Ads/OpenGateway ledger and onchain token/treasury transactions | partial | The aggregate buyback/burn output route is verified. Keep ad impressions, credits, USDC spend and `$GITLAWB` spend separate because no product-level attribution, retained-fee or margin ledger is verified. |
| GITLAWB bought and burned | Directly measures executed token demand/removal | Verified `GitlawbBuybackVault` contract reads and transfers | tested_ok | `180,964,104.671626 GITLAWB` processed/burned across `49` flushes as of 2026-07-25, up `90,819,704.850860` tokens and `23` flushes from July 22. The vault verifies output tokens and burn execution, not which product generated the USDC or the offchain swap's gross/net economics. |
...
| Buyback vault | Base `eth_call` plus Blockscout transfers at `0xA05174583791bA7fecb37278A4d5c97C1E24Fc5f` | tested_ok | `burnBps`, lifetime processed/burned/forwarded amounts, flush count, owner/sink and transfer receipts | Add collector history; separately map the upstream USDC wallet/swaps before labeling burns by Ads, OpenGateway or another product. |
...
- The canonical Base token is source-mapped as `0x5F980Dcfc4c0fa3911554cf5ab288ed0eb13DBa3`, and the buyback vault now proves `180.964M` tokens burned across `49` flushes. What are supply/owner/LP controls, LayerZero configuration, two-way delivery and the security/governance process for the owner-controlled burn ratio and future sink?
- Node/CLI v0.7.0's external workflow is verified and the broad adoption task is closed. Reopen only for a stable active/retained-user cohort, version-share metric or material external evaluation; registry totals and download requests are not unique active users.
...
- Which exact USDC inflows and swaps fund each verified buyback-vault flush, and how much comes from Ads, OpenGateway, x402 agents or other products after viewer/provider costs? The broad route is confirmed; product-level revenue attribution is not.
