...
| Stoa distributed RL post-training on consumer Macs | Confirmed / completed technical run | Official thread reports 14 Macs across four countries generated every rollout and PaperSearchQA pass@1 improved from 29% to 63%: https://x.com/Pluralis/status/2077419654578860539 | Completed run announced 2026-07-15 | Extends Pluralis from WAN pretraining into distributed agentic-RL rollouts, with DPPO handling off-policy drift and PULSE shrinking 9 GB updates to 82 MB. | A reproducible Stoa + Agora stack could broaden useful consumer-hardware participation beyond pretraining, but current single-Mac model fit and centralized-trainer limits remain. | Inspect the promised code/blog artifacts; reproduce PaperSearchQA results; verify Mac contribution, trainer dependency and whether rollout contributors receive enforceable economics. |
...
- Can third parties reproduce Stoa's 29%→63% PaperSearchQA result and inspect the full code, dataset/eval setup, trainer dependence and Mac contribution accounting?
