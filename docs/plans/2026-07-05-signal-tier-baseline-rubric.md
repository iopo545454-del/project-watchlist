...
**Date:** 2026-07-05
**Status (2026-07-05, verified):** Tasks 1 and 3 shipped in `60c40a8` — rubric text in AGENTS.md with the candidate-categories clause and protocol/funds scoping on security incidents; 12 retags (net `high` 58 → 48); GEODNET GIP-13 entries promoted to `high`; 0xSrMessi account compromise correctly held at `medium`. **Task 2 verified by Hermes on 2026-07-05** — the 4h cron scanner prompt contains the condensed baseline-relative rule and litmus test in the independent tagger instruction; the local `research-watchlist-dashboard` skill also carries the condensed rule. Rubric drift across AGENTS.md, cron, and skill is resolved as of this check.
