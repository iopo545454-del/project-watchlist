...
**Date:** 2026-07-05
**Status (2026-07-05, verified):** Tasks 1 and 3 shipped in `60c40a8` — rubric text in AGENTS.md with the candidate-categories clause and protocol/funds scoping on security incidents; 12 retags (net `high` 58 → 48); GEODNET GIP-13 entries promoted to `high`; 0xSrMessi account compromise correctly held at `medium`. **Task 2 is unverified from the repo** — the 4h cron scanner prompt and the `research-watchlist-dashboard` skill live outside this repository; Hermes must confirm both carry the condensed rule and note it here.
